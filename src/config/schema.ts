...
export const configSchema = z.object({
  ...
  // Stripe PUBLISHABLE key (TASK-215) — the `pk_test_…`/`pk_live_…` the browser needs to
  // construct Stripe.js for Embedded Checkout (inline, on-site payment). It is PUBLIC, not a
  // secret (it ships to every donor's browser), so it is injected as a plain env value, NOT an
  // SSM SecureString — but still required + non-empty (never defaulted), mirroring the other
  // Stripe keys, so a missing value fails boot rather than silently disabling inline checkout.
