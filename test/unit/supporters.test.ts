...
// TASK-023 (REQ-035): supporters.html's tiered supporters list. Three tier groups
// (Bronze, Silver, Gold, in that order) sit in the page-sections slot below the
// intro; entries are alphabetical within each tier, and each entry is marked as a
// person or an organisation (data-type + a visible kind label + a decorative
// aria-hidden icon, no <img>). The list lives in the HTML (SEO + perf budget +
// no-build; see docs/superpowers/specs/2026-07-01-supporters-list-design.md).
// Parsed with jsdom, mirroring home-pillars.test.ts / footer.test.ts. DB-free.
...
describe("supporters tiers (REQ-035)", () => {
  ...
  it("renders exactly three tier groups", () => {
    expect(tiers).toHaveLength(3);
  });

  it("labels the tiers Bronze, Silver, Gold in that order", () => {
